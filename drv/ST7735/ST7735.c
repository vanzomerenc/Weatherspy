...
uint32_t ST7735_DrawStringS(uint16_t x, uint16_t y, const char *pt, int16_t textColor, int16_t backgroundColor, uint8_t size){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6, y*10, *pt, textColor, backgroundColor, size);
    pt++;
    x = x+size;
    if(x>20) return count;  // number of characters printed
    count++;
